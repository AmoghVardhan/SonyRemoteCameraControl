...
int send_gps(int socket)
{
	...
	picture = fopen(name, "r");
   	printf("Getting GPS File Size\n");   
	if(picture == NULL) 
	{
        	printf("Error Opening GPS File"); 
	}
...
int main(int argc , char *argv[])
{
	while(1)
	{
		...
		if(count<=9)
		{
			...
			snprintf(gps,9,"gps%d.txt",count);
			//printf("IMAGE NAME:%s\n",name);
		}
		if(count>=10 && count<=99)
		{
			snprintf(name,13,"IMG_00%d.jpg",count);
			snprintf(gps,10,"gps%d.txt",count);
		}
		if(count>=100)
		{
			snprintf(name,13,"IMG_0%d.jpg",count);
			snprintf(gps,11,"gps%d.txt",count);
		}
	}
    	return 0;
}
